...
SAVE_DIR = Path(os.environ.get("OUTPUT_DIR", PROJECT_ROOT / "outputs" / "vae_nb_eval")).resolve()
SAVE_DIR.mkdir(parents=True, exist_ok=True)

# Default path for the NB model without zero inflation.
# You can still override this with: export WEIGHTS_PATH=/path/to/VAE_NB_weights-4-p-values
DEFAULT_WEIGHTS_PATH = PROJECT_ROOT / "outputs" / "VAE_NB_weights-4-p-values"
...
# Seed used for the single NB draw that goes into histogram plots.
# This keeps histogram sampling reproducible across runs.
NB_HIST_SAMPLE_SEED = 314159

# Fixed x / p baseline requested for the ablation.
# This is deliberately fixed to 1 / 0.19, not changed per p value.
SCALED_INPUT_BASELINE_P = 0.19
...
def vae_loss(
    ...
    This is the no-zero-inflation version of the evaluation loss.
    """
...
def scaled_input_baseline_019(x_corrupt):
    """
    Baseline requested for the ablation: multiply corrupted counts by 1 / 0.19.
    This is fixed to p=0.19, even when other p values are evaluated.
    """
    return np.asarray(x_corrupt, dtype=np.float32) / float(SCALED_INPUT_BASELINE_P)


# NB sampling helpers
def sample_nb_counts_np(mu_nb, theta_vec, rng, eps=1e-8):
    """
    Sample one count from a negative binomial distribution for every cell-gene entry.

    Parameterization:
      mean = mu_nb
      dispersion = theta
    implemented with Gamma-Poisson sampling.
    """
    ...
    return rng.poisson(lam).astype(np.float32, copy=False)


def nb_sample_once_batched(model, X_in, log_theta, device, batch_size=1024, rng_seed=0):
    """
    Run the model and draw a single NB sample for each cell-gene entry.
    ...
    with torch.inference_mode():
        for start in tqdm(range(0, X_in.shape[0], batch_size), desc="VAE 1x NB sampling", unit="batch"):
            ...
            sample_block = sample_nb_counts_np(mu_nb=mu_nb, theta_vec=theta_vec, rng=rng)
...
def nb_expected_counts_batched(model, X_in, device, batch_size=1024):
    """
    The decoder gives the NB mean directly after softplus.
    There is no zero-inflation multiplier in this NB ablation.
    """
    model.eval()
    out_blocks = []
    use_cuda = device.type == "cuda"
...
def expected_detection_rate_from_nb_mean(mu_nb, theta_vec, eps=1e-8):
    """
    Compute expected detection rate per gene from NB parameters.

    For an NB with mean mu and dispersion theta:
      P(X=0) = (theta / (theta + mu)) ** theta
      P(X>0) = 1 - P(X=0)
    """
    mu_nb = np.asarray(mu_nb, dtype=np.float32)
    theta_vec = np.asarray(theta_vec, dtype=np.float32)
...
def _method_acc_dict(methods):
    return {m: init_metric_acc() for m in methods}
...
def run_global_test_evaluation(
    ...
    print(f"Scaled baseline uses fixed multiplier: 1 / {SCALED_INPUT_BASELINE_P}")
    ...
    METHOD_VAE = "VAE (NB expected decode)"
    METHOD_ID = "Identity baseline (input passthrough)"
    METHOD_MEAN = "Gene-mean baseline (train)"
    METHOD_SCALE_019 = "Scaled input baseline (x / 0.19)"
    METHODS = [METHOD_VAE, METHOD_ID, METHOD_SCALE_019, METHOD_MEAN]

    overall_acc = _method_acc_dict(METHODS)
    ...
    for version_idx, p_val in enumerate(eval_p_values):
        ...
        for payload in eval_iter:
            ...
            xb_scaled_019_np = scaled_input_baseline_019(xb_np)
            ...
            with torch.inference_mode():
                ...
                recon_expected = F.softplus(recon).float().cpu().numpy().astype(np.float32, copy=False)
            ...
            preds = {
                METHOD_VAE: recon_expected,
                METHOD_ID: xb_np,
                METHOD_SCALE_019: xb_scaled_019_np,
                METHOD_MEAN: gene_mean_train[None, :],
            }

            for method_name, pred in preds.items():
                update_metric_acc(overall_acc[method_name], pred, yb_np)
                update_metric_acc(per_p_acc[(version_idx, method_name)], pred, yb_np)

                if split_name in {"A", "B", "C"}:
                    update_metric_acc(per_split_acc[(version_idx, split_name, method_name)], pred, yb_np)
                    update_metric_acc(per_panel_acc[(version_idx, split_name, sample_id, method_name)], pred, yb_np)

                    if split_name in {"B", "C"}:
                        valid_overlap_mask = sample_valid_overlap_mask_by_sid[sample_id]
                        if np.any(valid_overlap_mask):
                            update_metric_acc(
                                per_panel_valid_overlap_acc[(version_idx, split_name, sample_id, method_name)],
                                pred,
                                yb_np,
    ...
    for version_idx, p_val in enumerate(eval_p_values):
        sb = build_scoreboard_from_acc_dict({
            method_name: per_p_acc[(version_idx, method_name)]
...
def run_single_panel_gene_distribution_analysis(
    ...
    hist_rng = np.random.default_rng(
        int(NB_HIST_SAMPLE_SEED + version_idx * 100_003 + _stable_string_seed(sample_id))
    )
    ...
    for payload in tqdm(
        ...
        recon_expected = recon_mu
        recon_hist_sample = sample_nb_counts_np(
            mu_nb=recon_mu,
            theta_vec=theta_vec,
            rng=hist_rng,
        )
    ...
    theta_selected = theta_vec[selected_idx]
    detect_recon = expected_detection_rate_from_nb_mean(X_recon_expected, theta_selected)
    ...
    plot_selected_gene_histograms(
        ...
        label_recon=f"{sample_id} through VAE (1x NB sample)",
...
def run_split_panel_reconstruction_analysis(
    ...
    X_recon_expected_full = nb_expected_counts_batched(
        model=model,
        X_in=X_corrupt_full,
        device=device,
    ...
    det_recon = expected_detection_rate_from_nb_mean(X_recon_expected, theta_vec)
    ...
    save_reconstruction_scatter_suite(
        ...
        title_prefix=f"Split {split_name}: {sample_id} corrupted -> NB-VAE -> clean | valid train-overlap genes",
    ...
    plot_selected_gene_spatial_triplets(
        ...
        label_input=f"{sample_id} corrupted",
        label_recon=f"{sample_id} through NB-VAE",
        label_target=f"{sample_id} clean",
    )
...
def run_pair_analysis(
    ...
    X5k_recon_expected = nb_expected_counts_batched(
        model=model,
        X_in=X5k_in,
        device=device,
        batch_size=PAIR_INFER_BATCH_SIZE,
    ...
    X5k_recon_sampled = nb_sample_once_batched(
        model=model,
        X_in=X5k_in,
        log_theta=log_theta,
        device=device,
    ...
    det_recon = expected_detection_rate_from_nb_mean(X5k_recon_pair, theta_pair)
    ...
    save_reconstruction_scatter_suite(
        ...
        det_recon=det_recon,
        det_target=det_target,
        title_prefix=f"Pair analysis: {PAIR_5K_ID} -> NB-VAE -> {PAIR_V1_ID}",
    ...
    plot_selected_gene_histograms(
        ...
        label_recon=f"{PAIR_5K_ID} through NB-VAE (1x NB sample)",
...
def main():
    ...
    if ckpt.get("zero_inflation") is True or "logit_pi" in ckpt:
        print("Warning: checkpoint appears to contain zero-inflation information. This NB evaluation script ignores logit_pi.")

    print(
        f"Loaded NB checkpoint | input={input_dim_ckpt}, hidden={hidden_dim_ckpt}, latent={latent_dim_ckpt}"
    )

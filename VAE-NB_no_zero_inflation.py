...
class PanelRowAccessor:
    ...
    def __init__(self, panel_data, panel_ids, common_genes):
        ...
        self.common_genes = pd.Index(common_genes).astype(str)  # shared gene space
...
class MultiVersionTrainDataset(Dataset):
    def __init__(self, clean_matrix, p_non_overlap_values, base_seed=0):
        ...
        # Create a separate full-length p vector for each version of p
...
# Lower if process gets killed immediately
batch_size_cuda = 512
batch_size_cpu = 128
...
# Here we combine NB reconstruction loss with KL regularization.
# This version has no zero-inflation component.
def vae_loss(
    ...
    theta_param=None,  # Negative binomial dispersion parameter
    eps=1e-8,          # Helps with stability in log/exp calculations
):
    target_counts = target.clamp_min(0.0)
    mu_counts = F.softplus(recon) + eps

    # If not provided, use fixed theta=10 for every gene.
    # In this script, theta_param is provided, so theta is learned per gene.
...
# Learn per-gene NB dispersion theta.
# There is no zero-inflation parameter in this NB ablation.
...
optimizer = torch.optim.Adam(list(model.parameters()) + [log_theta], lr=learning_rate)
...
print(
    ...
    f"hidden_dim={hidden_dim}, lr={learning_rate}, recon_loss=NB(raw counts)"
)
...
for epoch in epoch_pbar:
    ...
    for xb, yb in train_iter:
        ...
        torch.nn.utils.clip_grad_norm_(list(model.parameters()) + [log_theta], max_norm=5.0)
...
print(f"Learned NB theta (median): {np.median(theta_learned):.4f}")
...
fig.suptitle("NB-VAE training vs validation curves", y=1.02)
plt.tight_layout()

save_dir = SAVE_DIR
save_dir.mkdir(parents=True, exist_ok=True)
loss_curve_path = save_dir / "VAE_NB_training_validation_curves.png"
...
save_dir = SAVE_DIR
save_dir.mkdir(parents=True, exist_ok=True)
weights_path = save_dir / "VAE_NB_weights-4-p-values"

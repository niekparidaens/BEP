...
# NB reconstruction loss + KL regularization
def vae_loss(
    ...
    # If not provided we say it's 10 for everything
    if theta_param is None:
        theta = torch.full((recon.shape[1],), 10.0, device=recon.device, dtype=recon.dtype)
    else:
        # If it is provided we make sure it's positive and learned
...
save_dir = SAVE_DIR
save_dir.mkdir(parents=True, exist_ok=True)
loss_curve_path = save_dir / "VAE_training_validation_curves_NB.png"
...
save_dir = SAVE_DIR
save_dir.mkdir(parents=True, exist_ok=True)
weights_path = save_dir / "VAE_NB_weights-NB-loss"

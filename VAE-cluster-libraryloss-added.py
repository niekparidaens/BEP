...
library_loss_weight = 1e-2  # try 0.0, 1e-3, 1e-2
...
class VAE(nn.Module):
    # Encoder-decoder VAE with explicit library-size scaling
    def __init__(self, input_dim, latent_dim=16, hidden_dim=256):
        ...
        # Instead of directly predicting counts, predict:
        # 1) gene composition (logits over genes)
        # 2) one library size per cell
        self.gene_logits = nn.Linear(hidden_dim, input_dim)
        self.log_library = nn.Linear(hidden_dim, 1)
    ...
    def decode(self, z):
        ...
        gene_logits = self.gene_logits(h)   # shape: [batch, n_genes]
        log_library = self.log_library(h)   # shape: [batch, 1]
        return gene_logits, log_library

    def forward(self, x):
        mu, logvar = self.encode(x)
        z = self.reparameterize(mu, logvar)
        gene_logits, log_library = self.decode(z)
        return gene_logits, log_library, mu, logvar


# ZINB reconstruction loss + KL + optional library-size supervision
def vae_loss(
    gene_logits,      # Decoder gene-composition logits
    log_library,      # Decoder predicted log library size
    target,           # Target raw counts
    mu,               # Encoder latent mean vectors
    logvar,           # Encoder latent log-variance vectors
    beta=1e-3,
    loss_mask=None,
    theta_param=None,
    zi_logits=None,
    eps=1e-8,
    library_loss_weight=1e-2,
):
    target_counts = target.clamp_min(0.0)

    # Explicit library-size scaling:
    # proportions across genes * total library size per cell
    gene_props = torch.softmax(gene_logits, dim=1)
    library_size = torch.exp(log_library)               # [batch, 1]
    mu_counts = library_size * gene_props + eps         # [batch, n_genes]

    if theta_param is None:
        theta = torch.full((mu_counts.shape[1],), 10.0, device=mu_counts.device, dtype=mu_counts.dtype)
    else:
    ...
    # Optional supervision on total target depth
    target_library = target_counts.sum(dim=1, keepdim=True)
    library_loss = F.mse_loss(log_library, torch.log1p(target_library))

    total_loss = recon_loss + beta * kl + library_loss_weight * library_loss
    return total_loss, recon_loss, kl, library_loss
...
print(
    ...
    f"recon_loss=ZINB(raw counts, library-size scaled), library_loss_weight={library_loss_weight}"
)
...
for epoch in epoch_pbar:
    ...
    for xb, yb in train_iter:
        ...
        with torch.cuda.amp.autocast(enabled=use_cuda):
            gene_logits, log_library, mu, logvar = model(xb)
            loss, recon_loss, kl, lib_loss = vae_loss(
                gene_logits,
                log_library,
                yb,
                mu,
                ...
                zi_logits=logit_pi,
                library_loss_weight=library_loss_weight,
            )
        ...
        if n_train_batches % 50 == 0:
            train_iter.set_postfix({
                ...
                "lib": f"{(train_lib_sum / n_train_batches):.4f}",
            })
    ...
    epoch_train_lib = train_lib_sum / n_train_batches
    ...
    train_lib_history.append(epoch_train_lib)
    ...
    with torch.no_grad():
        for xb, yb in val_loader:
            ...
            with torch.cuda.amp.autocast(enabled=use_cuda):
                gene_logits, log_library, mu, logvar = model(xb)
                val_loss, val_recon, val_kl, val_lib = vae_loss(
                    gene_logits,
                    log_library,
                    yb,
                    mu,
    ...
    if epoch % 5 == 0 or epoch == 1 or improved:
        print(
            ...
            f"train_lib={epoch_train_lib:.6f} | val_lib={epoch_val_lib:.6f}"
        )
